 ...
 *
 *  Tauri port: This is the main workbench bootstrap entry point, adapted to run
 *  under Tauri instead of Electron. All IPC and platform access flows through the
 *  preload bridge (`window.vscode`), which internally uses Tauri invoke/listen.
 *--------------------------------------------------------------------------------------------*/
...
(async function () {
	...
	// The preload bridge populates `window.vscode` with ipcRenderer, webFrame,
	// process, and context — backed by Tauri invoke/listen under the hood.
	...
	function showDefaultSplash(configuration: INativeWindowConfiguration) {
		...
		// Set zoom level as soon as possible (routed through the Tauri preload bridge)
	...
	async function load<M, T extends ISandboxConfiguration>(options: ILoadOptions<T>): Promise<ILoadResult<M, T>> {
		...
		// Window configuration from the Tauri preload bridge
	...
	async function resolveWindowConfiguration<T extends ISandboxConfiguration>() {
		...
		// Resolved via the Tauri preload bridge (invoke('resolve_configuration'))
	...
	function registerDeveloperKeybindings(disallowReloadKeybinding: boolean | undefined): Function {
		// IPC calls go through the preload bridge, which routes them via Tauri invoke
		...
		// Devtools & reload support (keybindings routed through the Tauri preload bridge)
	...
	function onUnexpectedError(error: string | Error, showDevtoolsOnError: boolean): void {
		if (showDevtoolsOnError) {
			// Opens devtools via the Tauri preload bridge
			const ipcRenderer = preloadGlobals.ipcRenderer;

...
import { TauriDisplay } from './windows.js';
...
export class WindowsStateHandler extends Disposable {
	...
	private registerListeners(): void {
		...
		// When a window loses focus, save all windows state.
		// In Tauri, we listen for focus changes via invoke/listen pattern
		invoke('on_window_blur_subscribe').catch(() => { /* optional */ });
		const blurInterval = setInterval(() => {
			if (!this.shuttingDown) {
				// Periodically save state as a safety measure
			}
		}, 30000);
		this._register({ dispose: () => clearInterval(blurInterval) });
		...
		if (extensionHostWindow) {
			...
			if (currentWindowsState.lastPluginDevelopmentHostWindow.uiState.mode === WindowMode.Fullscreen) {
				if (displaysWithFullScreenWindow.has(currentWindowsState.lastPluginDevelopmentHostWindow.uiState.display)) {
					if (isMacintosh) {
						// Tauri has no simple fullscreen concept; treat all fullscreen as native
	...
	private async doGetNewWindowState(configuration: INativeWindowConfiguration): Promise<INewWindowState> {
		...
		let displayToUse: TauriDisplay | undefined;
		let displays: TauriDisplay[] = [];
		...
		// Multi Display
			...
			if (isMacintosh) {
				try {
					displayToUse = await invoke<TauriDisplay>('get_display_at_cursor');
			...
			if (!displayToUse && lastActive) {
				try {
					displayToUse = await invoke<TauriDisplay>('get_display_matching', { rect: lastActive.getBounds() });
				} catch { /* fallback below */ }
			}
			...
			if (!displayToUse) {
				try {
					displayToUse = await invoke<TauriDisplay>('get_primary_display');
				} catch { /* fallback */ }
				if (!displayToUse) {
					displayToUse = displays[0];
				}

...
export class WindowsMainService extends Disposable implements IWindowsMainService {
	...
	private focusMainOrChildWindow(mainWindow: ICodeWindow): void {
		...
		// In Tauri, we use invoke to check the focused window
		invoke<string | null>('get_focused_window_label').then(focusedLabel => {
			if (focusedLabel && focusedLabel !== mainWindow.win?.label) {
				const auxiliaryWindowCandidate = this.auxiliaryWindowsMainService.getWindowByWebContents(focusedLabel);
				if (auxiliaryWindowCandidate && auxiliaryWindowCandidate.parentId === mainWindow.id) {
					auxiliaryWindowCandidate.focus();
					return;
				}
			}
			mainWindow.focus();
	...
	private async onUNCHostNotAllowed(path: string, options: IPathResolveOptions): Promise<IPathToOpen<ITextEditorOptions> | undefined> {
		...
		if (response === 2) {
			shellOpen('https://aka.ms/vscode-windows-unc');
	...
	private async openInBrowserWindow(options: IOpenBrowserWindowOptions): Promise<ICodeWindow> {
		...
		const configuration: INativeWindowConfiguration = {
			...
			accessibilitySupport: await invoke<boolean>('is_accessibility_support_enabled'),
		...
		if (!window) {
			...
			// In Tauri, devtools reload is handled via the Rust backend
			disposables.add(toDisposable(() => { /* devtools-reload-page cleanup */ }));
	...
	getFocusedWindow(): ICodeWindow | undefined {
		// In Tauri we track focus via lastFocusTime since there's no sync API

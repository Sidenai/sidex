...
type ProtocolCallback = { (result: string | { path: string; headers?: Record<string, string> } | { error: number }): void };
...
export class ProtocolMainService extends Disposable implements IProtocolMainService {
	...
	private handleProtocols(): void {
		// Tauri stub: Protocol handling is done via Tauri's protocol plugin
		// Register protocol handlers through Rust backend via invoke()
		invoke('register_protocol', { scheme: Schemas.vscodeFileResource }).catch(() => {});
	}
	...
	private handleFileRequest(request: { url: string }, callback: ProtocolCallback) {
	...
	private handleResourceRequest(request: { url: string }, callback: ProtocolCallback): void {
	...
	private requestToNormalizedFilePath(request: { url: string }): string {

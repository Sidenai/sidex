...
type JumpListCategory = any;
type JumpListItem = any;
...
export class WorkspacesHistoryMainService extends Disposable implements IWorkspacesHistoryMainService {
			...
			// File
				...
				if (!alreadyExistsInHistory && !shouldBeFiltered) {
					...
					if (isWindows && recent.fileUri.scheme === Schemas.file && !this.environmentMainService.isPortable) {
						invoke('add_recent_document', { path: recent.fileUri.fsPath }).catch(() => {});
					}
				}
			}
		}
	...
	async clearRecentlyOpened(options?: { confirm?: boolean }): Promise<void> {
		...
		if (!this.environmentMainService.isPortable) {
			invoke('clear_recent_documents').catch(() => {});
		}
	...
	private async updateWindowsJumpList(): Promise<void> {
		...
		if ((await this.getRecentlyOpened()).workspaces.length > 0) {
			...
			const toRemove: URI[] = [];
			// Tauri stub: Jump list not available. Would need a native Windows API call.
			for (const item of []) {
		...
		try {
			// Tauri stub: setJumpList is a Windows-specific Electron API
			// In Tauri, this would be done via a Rust plugin
			await invoke('set_jump_list', { jumpList }).catch(() => {});
		} catch (error) {
	...
	private async updateMacOSRecentDocuments(): Promise<void> {
		...
		fileEntries.reverse().forEach(fileEntry => invoke('add_recent_document', { path: fileEntry }).catch(() => {}));
		workspaceEntries.reverse().forEach(workspaceEntry => invoke('add_recent_document', { path: workspaceEntry }).catch(() => {}));
	}

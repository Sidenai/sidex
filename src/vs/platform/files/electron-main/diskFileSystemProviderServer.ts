...
async function trashItem(filePath: string): Promise<void> {
	// Tauri stub: use plugin-opener or invoke Rust backend for trash functionality
	await import('@tauri-apps/api/core').then(({ invoke }) => invoke('trash_item', { path: filePath }));
}

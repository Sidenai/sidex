...
export class ElectronURLListener extends Disposable {
	...
	constructor(
		...
		// Tauri stub: protocol handler registration is handled by Tauri's deep-link plugin
		if (isWindows && !environmentMainService.isPortable && !(process as INodeProcess).isEmbeddedApp) {
			invoke('register_protocol_handler', { protocol: productService.urlProtocol }).catch(() => {});
		}

		// macOS: listen to open-url events via Tauri deep-link plugin

...
export interface TauriRectangle {
	x: number;
	y: number;
	width: number;
	height: number;
...
export interface IBaseWindow extends IDisposable {
	...
	readonly id: number;
	readonly win: WebviewWindow | null;
...
export const defaultAuxWindowState = async function (): Promise<IWindowState> {
	...
	const workArea: TauriRectangle = await invoke('get_primary_display_work_area');

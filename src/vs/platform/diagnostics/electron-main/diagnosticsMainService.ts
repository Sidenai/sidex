...
export class DiagnosticsMainService implements IDiagnosticsMainService {
	...
	async getMainDiagnostics(): Promise<IMainProcessDiagnostics> {
		...
		type AppWithGPULogMethod = {
			getGPULogMessages(): IGPULogMessage[];
		};

		let gpuLogMessages: IGPULogMessage[] = [];
		// Tauri stub: GPU log messages not available
		const accessibilitySupportEnabled = await invoke<boolean>('is_accessibility_enabled').catch(() => false);
		const gpuFeatureStatus = await invoke<any>('get_gpu_feature_status').catch(() => ({}));
		...
		return {
			...
			windows,
			pidToNames,
			screenReader: accessibilitySupportEnabled,
			gpuFeatureStatus,
			gpuLogMessages
		};
	}

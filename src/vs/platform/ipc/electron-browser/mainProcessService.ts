...
/**
 * An implementation of `IMainProcessService` that leverages the Tauri IPC bridge.
 *
 * Under the hood, the `IPCElectronClient` uses the preload bridge's
 * `ipcRenderer.send/on` which route through Tauri's `invoke()` / `listen()`.
 */

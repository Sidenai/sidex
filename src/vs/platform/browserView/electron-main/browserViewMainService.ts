...
type Menu = any;
type MenuItem = any;
const clipboard = { write(_data: any): void {}, writeText(_text: string): void {} };
const MenuConstructor = class { items: any[] = []; append(_item: any): void {} popup(_opts: any): void {} };
const MenuItemConstructor = class { constructor(_opts: any) {} };
...
export class BrowserViewMainService extends Disposable implements IBrowserViewMainService {
	 ...
	 */
	static isBrowserViewWebContents(contents: any): boolean {
	 ...
	 */
	private createBrowserView(id: string, browserSession: BrowserSession, options?: any): BrowserView {
		if (this.browserViews.has(id)) {
	...
	private showContextMenu(view: BrowserView, params: any): void {
		...
		if (params.linkURL) {
			menu.append(new MenuItemConstructorConstructor({
			...
			menu.append(new MenuItemConstructor({
			...
			menu.append(new MenuItemConstructor({ type: 'separator' }));
			menu.append(new MenuItemConstructor({
		...
		if (params.isEditable) {
			menu.append(new MenuItemConstructor({ role: 'cut', enabled: params.editFlags.canCut }));
			menu.append(new MenuItemConstructor({ role: 'copy', enabled: params.editFlags.canCopy }));
			menu.append(new MenuItemConstructor({ role: 'paste', enabled: params.editFlags.canPaste }));
			menu.append(new MenuItemConstructor({ role: 'pasteAndMatchStyle', enabled: params.editFlags.canPaste }));
			menu.append(new MenuItemConstructor({ role: 'selectAll', enabled: params.editFlags.canSelectAll }));
		} else if (params.selectionText) {
			menu.append(new MenuItemConstructor({ role: 'copy' }));
		}

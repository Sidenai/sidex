...
export class BrowserSession {
	 ...
	 */
	private static readonly _bySession = new WeakMap<any, BrowserSession>();
	 ...
	 */
	static readonly knownSessions = new WeakSet<any>();
	...
	static getOrCreateGlobal(): BrowserSession {
		// Tauri stub: No session.fromPartition(). Create a stub session object.
		const electronSession = { id: 'persist:vscode-browser' } as any;
	...
	static getOrCreateWorkspace(workspaceId: string, workspaceStorageHome: URI): BrowserSession {
		...
		const electronSession = { id: `workspace:${workspaceId}`, path: storage.fsPath } as any;
		return BrowserSession._bySession.get(electronSession)
	...
	static getOrCreateEphemeral(viewId: string, type?: string): BrowserSession {
		...
		const electronSession = { id: sessionId } as any;
		return BrowserSession._bySession.get(electronSession)
	...
	private constructor(
		 ...
		 */
		readonly id: string,
		/** The underlying session (Tauri stub). */
		readonly electronSession: any,
	...
	private configure(): void {
		// Tauri stub: permission handling would be done via Rust backend
		// No session.setPermissionRequestHandler in Tauri
	}
	...
	async clearData(): Promise<void> {
		...
		// Tauri stub: no electronSession.clearData() available
	}

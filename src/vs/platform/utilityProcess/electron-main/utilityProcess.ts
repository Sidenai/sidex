...
type MessageChannelMain = { port1: any; port2: any };
type ElectronUtilityProcess = any;
type Details = { type: string; name: string; exitCode: number; reason: string };
...
export class UtilityProcess extends Disposable {
	...
	protected doStart(configuration: IUtilityProcessConfiguration): boolean {
		...
		const jsFlags = await invoke<string>('get_command_line_switch', { name: 'js-flags' }).catch(() => '');
		if (jsFlags) {
		...
		// Tauri stub: fork utility process via Rust sidecar or Node child_process
		// In Tauri, there's no electron utilityProcess.fork().
		// We use a child_process.fork() as a fallback.
		const { fork } = await import('child_process');
		this.process = fork(modulePath, args, {
			env,
			execArgv,
			stdio: stdio as any
		}) as any;
	...
	private registerListeners(process: ElectronUtilityProcess, configuration: IUtilityProcessConfiguration, serviceName: string): void {
		...
		// Child process gone
		// Tauri stub: listen for child process crash via process events
		this._register(Event.fromNodeEventEmitter<{ code: number }>(process, 'exit', (code) => ({ code }))(({ code }) => {
			if (code !== 0) {
				this.log(`process exited with code ${code}`, Severity.Error);
			}
		}));
	}
	...
	postMessage(message: unknown, transfer?: any[]): boolean {
		if (!this.process) {
	...
	connect(payload?: unknown): any {
		// Tauri stub: MessageChannelMain is not available.
		// Use a Node.js MessageChannel or custom IPC mechanism.
		const { MessageChannel } = require('worker_threads');
		const { port1: outPort, port2: utilityProcessPort } = new MessageChannel();

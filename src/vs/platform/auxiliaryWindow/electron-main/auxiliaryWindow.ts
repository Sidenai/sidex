...
export class AuxiliaryWindow extends BaseWindow implements IAuxiliaryWindow {
	...
	private doTryClaimWindow(options?: BrowserWindowConstructorOptions): void {
		...
		// Tauri stub: In Tauri, there's no BrowserWindow.fromWebContents().
		// Window management is done via the Tauri window API.
		// We use invoke() to find a window associated with this webContents.
		const window = invoke<any>('get_window_by_webcontents', { webContentsId: this.webContents.id }).catch(() => null);
		if (window) {

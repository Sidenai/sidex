...
export class Win32UpdateService extends AbstractUpdateService implements IRelaunchHandler {
	...
	protected override async initialize(): Promise<void> {
		...
		if (this.productService.win32VersionedUpdate) {
			...
			// Tauri stub: app.setPath not available, cache path managed differently
			await invoke('set_update_cache_path', { path: cachePath }).catch(() => {});
	...
	protected override async postInitialize(): Promise<void> {
		...
		const exePath = await invoke<string>('get_exe_path').catch(() => process.execPath);

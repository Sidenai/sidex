...
// Tauri stub: no autoUpdater available from Electron
const autoUpdater = {
	setFeedURL(_opts: any): void {},
...
export class DarwinUpdateService extends AbstractUpdateService implements IRelaunchHandler {
	...
	@memoize private get onRawError(): Event<string> { return Event.fromNodeEventEmitter(autoUpdater as any, 'error', (_, message) => message); }
	@memoize private get onRawCheckingForUpdate(): Event<void> { return Event.fromNodeEventEmitter<void>(autoUpdater as any, 'checking-for-update'); }
	@memoize private get onRawUpdateNotAvailable(): Event<void> { return Event.fromNodeEventEmitter<void>(autoUpdater as any, 'update-not-available'); }
	@memoize private get onRawUpdateAvailable(): Event<void> { return Event.fromNodeEventEmitter(autoUpdater as any, 'update-available'); }
	@memoize private get onRawUpdateDownloaded(): Event<IUpdate> {
		return Event.fromNodeEventEmitter(autoUpdater as any, 'update-downloaded', (_, version: string, productVersion: string, releaseDate: Date | number) => ({
			version,
			productVersion,
	...
	protected buildUpdateFeedUrl(quality: string, commit: string, options?: IUpdateURLOptions): string | undefined {
		...
		try {
			this.logService.trace('update#buildUpdateFeedUrl - setting feed URL for autoUpdater', { url, assetID, quality, commit, headers });
	...
	protected doCheckForUpdates(explicit: boolean, pendingCommit?: string): void {
		...
		this.logService.trace('update#doCheckForUpdates - using autoUpdater', { url, explicit, background });

...
export function findFreePort(startPort: number, giveUpAfter: number, timeout: number, stride = 1): Promise<number> {
	...
	return new Promise(resolve => {
		...
		invoke<number>('net_find_free_port', { startPort, giveUpAfter, stride })
			.then(port => {
				if (!done) {
					done = true;
...
export function isPortFree(port: number, timeout: number): Promise<boolean> {
	return findFreePortFaster(port, 0, timeout).then(p => p !== 0);
}

/**
 * Uses listen instead of connect. Is faster, but if there is another listener
 * on 0.0.0.0 then this will take 127.0.0.1 from that listener.
 */
export function findFreePortFaster(startPort: number, giveUpAfter: number, timeout: number, hostname: string = '127.0.0.1'): Promise<number> {
	...
	return new Promise<number>(resolve => {
		...
		invoke<number>('net_find_free_port_fast', { startPort, giveUpAfter, hostname })
			.then(port => {
				if (!resolved) {
					resolved = true;
					if (timeoutHandle) {
